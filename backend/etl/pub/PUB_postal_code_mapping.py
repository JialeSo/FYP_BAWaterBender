...
def extract_location_parts_and_cleaned(
    raw: str,
...
# -----------------------
def jitter_coords(
    lat: float, lon: float, radius_m: float = JITTER_RADIUS_METERS
) -> Tuple[float, float]:
...
def geocode_location_with_variants(
    part: str, parent_context: Optional[str] = None
...
def reverse_geocode_postcode_with_buffer(
    lat: float, lon: float, attempts: int = MAX_REVERSE_JITTER_ATTEMPTS
) -> Optional[str]:
    ...
    for attempt in range(attempts):
        try_lat, try_lon = (
            (lat, lon)
            if attempt == 0
            else jitter_coords(lat, lon, radius_m=JITTER_RADIUS_METERS)
        )
...
def enrich_df_keep_original(df_in: pd.DataFrame) -> pd.DataFrame:
    ...
    df["start_loc"] = location_data.apply(
        lambda x: x[0][0] if x[0] and len(x[0]) > 0 else None
    )
    df["end_loc"] = location_data.apply(
        lambda x: x[0][1] if x[0] and len(x[0]) > 1 else None
    )
...
if __name__ == "__main__":
    if not infile.exists():
        ...
        safe_print(
            f"Files in directory: {[f.name for f in BASE.iterdir() if f.is_file()]}"
        )
        raise SystemExit(1)
    ...
    # Drop 'start', 'end', 'created_at', 'sender_id', 'id' if they exist
    df = df.drop(
        columns=[
            col
            for col in ["start", "end", "created_at", "sender_id", "id"]
    ...
    if "created_at" in df.columns:
        df.rename(columns={"created_at": "event_date"}, inplace=True)
    elif "event_date_time" in df.columns:
        # Legacy support for old field name
        df.rename(columns={"event_date_time": "event_date"}, inplace=True)
    df["event_date"] = pd.to_datetime(df["event_date"], errors="coerce").dt.strftime(
        "%Y-%m-%d"
    )
