...
class GeocodePostalOneMapStage(PipelineStage):
    ...
    def _onemap_elastic_search_postal(
        self, postal: str
    ) -> Tuple[Optional[float], Optional[float]]:
    ...
    async def process(self, data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if self.postal_lookup:
            ...
            df_out.loc[fill_mask, "_lat_fill"] = df_out.loc[
                fill_mask, "postal_code"
            ].map(lat_map)
            df_out.loc[fill_mask, "_lon_fill"] = df_out.loc[
                fill_mask, "postal_code"
            ].map(lon_map)
            ...
            df_out = df_out.drop(
                columns=[c for c in ["_lat_fill", "_lon_fill"] if c in df_out.columns]
            )
        ...
        unmatched_postals = (
            df_out.loc[unmatched_idx, "postal_code"]
            .astype(str)
            .str.strip()
            .unique()
            .tolist()
        ...
        if results_cache:
            df_out.loc[unmatched_idx, "_lat_fill_api"] = df_out.loc[
                unmatched_idx, "postal_code"
            ].map(lambda p: results_cache.get(p, (None, None))[0])
            df_out.loc[unmatched_idx, "_lon_fill_api"] = df_out.loc[
                unmatched_idx, "postal_code"
            ].map(lambda p: results_cache.get(p, (None, None))[1])
            # Align mask to the full DataFrame; do not boolean-index unmatched_idx (length mismatch)
            have_both = (
                df_out["_lat_fill_api"].notna() & df_out["_lon_fill_api"].notna()
            )
            assign_mask = have_both & df_out.index.isin(unmatched_idx)
            df_out.loc[assign_mask, "latitude"] = df_out.loc[
                assign_mask, "_lat_fill_api"
            ]
            df_out.loc[assign_mask, "longitude"] = df_out.loc[
                assign_mask, "_lon_fill_api"
            ]
            df_out = df_out.drop(
                columns=[
                    c for c in ["_lat_fill_api", "_lon_fill_api"] if c in df_out.columns
                ]
            )
            ...
            try:
                ...
                if newly_resolved:
                    ...
                    for item in newly_resolved:
                        self.postal_lookup[item["postal"]] = (
                            item["latitude"],
                            item["longitude"],
                        )
                    ...
                    try:
                        if self.postal_ref_csv.exists():
                            ...
                            existing.columns = [
                                c.strip().lower() for c in existing.columns
                            ]
                        else:
                            existing = pd.DataFrame(
                                columns=["postal", "latitude", "longitude"]
                            )
                        add_df = pd.DataFrame(newly_resolved)
                        add_df["postal"] = (
                            add_df["postal"].astype(str).str.strip().str.zfill(6)
                        )
                    ...
                    try:
                        ...
                        logger.info(
                            "Upserted new postal entries to Supabase (onemap_postal_codes)"
                        )
                    except Exception as e:
        ...
        try:
            ...
            cols = [c for c in ordered if c in df_out.columns] + [
                c for c in df_out.columns if c not in ordered
            ]
            df_out = df_out[cols]
            self.out_dir.mkdir(parents=True, exist_ok=True)
            ...
            unmatched = unmatched[
                unmatched["postal_code"].astype(str).str.fullmatch(r"\d{6}") == True
            ]
            unmatched_path = self.out_dir / "acra_unmatched_postal.csv"
            unmatched.to_csv(unmatched_path, index=False)
            logger.info(
                f"Wrote unmatched postal rows → {unmatched_path} ({len(unmatched)} rows)"
            )
        except Exception as e:
            logger.warning(f"Failed to write updated CSV: {e}")

        logger.info(
            "OneMap geocoding summary: CSV=%s, API=%s, Miss=%s",
            csv_hits,
            api_hits,
            misses,
        )
        ...
        try:
            ...
            logger.info(
                f"  • pa_id populated: {df_out['pa_id'].notna().sum():,} / {len(df_out):,}"
            )
            logger.info(
                f"  • sz_id populated: {df_out['sz_id'].notna().sum():,} / {len(df_out):,}"
            )
            logger.info(
                f"  • rn_id populated: {df_out['rn_id'].notna().sum():,} / {len(df_out):,}"
            )
            logger.info("=" * 80)
            ...
            if filtered_count > 0:
                logger.info(
                    f"Filtered out {filtered_count:,} records with excluded pa_id (24, 27, 31)"
                )
            ...
            logger.warning(
                "Continuing without three-layer geocoding (pa_id, sz_id, rn_id will be null)"
            )
        ...
        logger.info(
            f"Prepared {len(df_out):,} records for database upsert (ID will be auto-generated)"
        )
