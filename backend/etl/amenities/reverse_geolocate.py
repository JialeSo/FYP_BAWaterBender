...
class SGReverseGeolocator:
    def __init__(
        self, flood_csv, planning_geojson, subzone_geojson, road_network_geojson
    ):
    ...
    def reverse_lookup(self, postal_code=None, lat=None, lon=None):
        ...
        if lat is not None and lon is not None and pd.notna(lat) and pd.notna(lon):
            ...
            and "Postal_Code" in self.flood_gdf.columns
        ):
...
def process_location(
    row, prefix, postal_col, lat_col, lng_col, geo: SGReverseGeolocator
):
    """Process a location (start or end) and return prefixed columns with IDs."""
    if all(
        col not in row or pd.isna(row[col]) or row[col] == ""
        for col in [postal_col, lat_col, lng_col]
    ):
        return {
            f"{prefix}planning_area": "",
            f"{prefix}planning_area_id": "",
            f"{prefix}subzone": "",
            f"{prefix}subzone_id": "",
    ...
    res = geo.reverse_lookup(
        postal_code=row.get(postal_col), lat=row.get(lat_col), lon=row.get(lng_col)
    )
...
if __name__ == "__main__":
    geo = SGReverseGeolocator(
        FLOOD_PRECIP_CSV, PLANNING_GEOJSON, SUBZONE_GEOJSON, ROAD_NETWORK_GEOJSON
    )

    # Read CSV with postal codes as strings
    flood_df = pd.read_csv(
        FLOOD_PRECIP_CSV,
        dtype={"Postal_Code": str, "start_postal_code": str, "end_postal_code": str},
    )
    ...
    for _, row in flood_df.iterrows():
        ...
        row_result.update(
            process_location(
                row, "start_", "start_postal_code", "start_lat", "start_lng", geo
            )
        )
        row_result.update(
            process_location(row, "end_", "end_postal_code", "end_lat", "end_lng", geo)
        )
    ...
    enriched_df = enriched_df.drop(
        columns=["created_at", "sender_id", "id"], errors="ignore"
    )
