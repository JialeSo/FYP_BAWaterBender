...
router = APIRouter(prefix="/planning-area", tags=["planning-area"])
...
# JSON
# get all planning area data
@router.get("/")
async def get_all_planning_area_data():     
    try:
        result = db.table("planning_area").select("*").execute()
        ...
        logger.error(f"❌ Error fetching planning_area data: {e}")
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")

# GeoJSON
@router.get("/geojson")
async def get_planning_area_geojson():  
    try:
        result = db.table("planning_area").select("*").execute()
        
        if not result.data:
            logger.info("No planning area data found")
            return {"type": "FeatureCollection", "features": [], "count": 0}

        return to_geojson(result)
            
    except Exception as e:
        logger.error(f"❌ Error fetching planning_area geojson data: {e}")
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")