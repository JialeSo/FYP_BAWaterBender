...
function enrich_floods(floods_fc, lookups) {
  ...
  const features = (floods_fc.features || []).map((f) => {
    ...
    const start_pa = p.start_pa_id ? planning.by_id[p.start_pa_id] : null;
    const start_sz = p.start_sz_id ? subzone.by_id[p.start_sz_id] : null;
    const start_rn = p.start_rn_id ? road.by_id[p.start_rn_id] : null;
    ...
    p.start_planning_area = start_pa?.name ?? null;
    p.start_subzone       = start_sz?.name ?? null;
    p.start_road          = start_rn?.name ?? null;
    p.end_planning_area   = end_pa?.name ?? null;
    p.end_subzone         = end_sz?.name ?? null;
    p.end_road            = end_rn?.name ?? null;

    const origin = [to_num(p.start_lng), to_num(p.start_lat)];
    ...
    p.segment      = p.has_segment ? { type: "LineString", coordinates: [p.origin_point, p.end_point] } : null;
...
function mapdataprovider({ children }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        set_loading(true);
        const [planning, subzone, road, floods_csv, amenity_csv] = await Promise.all([
  ...
  /* ========= enrich floods ========= */
  const floods_fc_enriched = useMemo(() => enrich_floods(floods_fc_raw || { type: "FeatureCollection", features: [] }, lookups), [floods_fc_raw, lookups]);
  ...
  const road_fc_enriched = useMemo(() => {
    ...
    for (const f of floods_fc_enriched.features || []) {
      const p = f.properties || {};
      for (const rn_id of [p.start_rn_id, p.end_rn_id].filter(x => x != null)) {
        flood_counts[rn_id] = (flood_counts[rn_id] || 0) + 1;
      }
    }

    const amenity_counts = Object.create(null);
    for (const f of amenity_fc_raw?.features || []) {
      const p = f.properties || {};
      if (p.rn_id != null) {
    ...
    return { type: "FeatureCollection", features };
  }, [road_fc_raw, floods_fc_enriched, amenity_fc_raw]);
  ...
  useEffect(() => {
    console.log("🧭 Loaded data summary:", {
      ...
      floods: floods_fc_raw?.features?.length,
      amenities: amenity_fc_raw?.features?.length,
    });
  }, [planning_fc_raw, subzone_fc_raw, road_fc_raw, floods_fc_raw, amenity_fc_raw]);
...
export { mapdataprovider as MapDataProvider, usemapdata as useMapData };
