...
import { Check, ChevronsUpDown, X, Download, Info } from "lucide-react";
...
/* ===== map component ===== */
function CentralityMap({ data, selectedRoadId, onMapLoad }) {
...
/* ===== Road List Panel ===== */
function RoadListPanel({ roads, selectedRoadId, onSelectRoad }) {
  const [searchQuery, setSearchQuery] = useState("");

  const filteredRoads = useMemo(() => {
    const q = searchQuery.trim().toLowerCase();
    if (!q) return roads;
    return roads.filter(f => {
      const p = f.properties;
      const searchStr = [
        p.name,
        p.PLN_AREA_N,
    ...
    <div className="h-full flex flex-col bg-card border rounded-2xl shadow-sm">
      ...
      <div className="p-4 border-b">
        ...
        <p className="text-xs text-muted-foreground mt-2">
          {filteredRoads.length.toLocaleString()} roads
        </p>
      </div>
      ...
      <ScrollArea className="flex-1">
        <div className="p-2 space-y-1">
          {filteredRoads.map((road, idx) => {
            ...
            const isSelected = p.RN_ID === selectedRoadId;
            
            return (
              <button
                key={p.RN_ID || idx}
                onClick={() => onSelectRoad(p.RN_ID)}
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  <span className="text-xs shrink-0">#{idx + 1}</span>
                </div>
                
                <div className="grid grid-cols-2 gap-x-2 gap-y-0.5 text-xs opacity-90">
                  <div>
                    <span className="opacity-70">Score:</span>{" "}
                    <strong>{format_number(p.importance, 1)}</strong>
                  </div>
                  <div>
                    <span className="opacity-70">ID:</span> {p.RN_ID}
                  </div>
                  <div className="col-span-2 truncate">
                    <span className="opacity-70">Area:</span> {p.PLN_AREA_N || "—"}
                  </div>
                </div>
              </button>
            );
...
/* ===== Road Details Panel ===== */
function RoadDetailsPanel({ road, onClose, amenityCounts, floodCounts }) {
  if (!road) return null;

  const p = road.properties;
  
  // Get amenity breakdown
  ...
  const floodBreakdown = useMemo(() => {
    const result = [];
    if (floodCounts) {
      Object.entries(floodCounts).forEach(([type, count]) => {
        if (count > 0) {
              ...
              <ScrollArea className="h-32">
                <div className="space-y-1.5 pr-2">
                  {amenityBreakdown.map(({ category, count }) => (
                    <div 
                      key={category}
                      className="flex items-center justify-between text-xs rounded px-2 py-1.5 bg-muted/50"
                    >
